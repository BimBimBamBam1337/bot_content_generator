...
async def handle_robokassa_result(request: web.Request):
    text = await request.text()  # POST данные от Robokassa
    response = result_payment(text)
    return web.Response(text=response)


async def handle_robokassa_success(request: web.Request):
    text = str(request.query_string)  # GET параметры после оплаты
    response = check_success_payment(text)
    return web.Response(text=response)


def setup_robokassa_routes(app: web.Application):
